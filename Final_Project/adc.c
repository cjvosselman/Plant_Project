...
void ADC0_init(uint32_t reference) {
  ...
  ADC0->ULLMEM.GPRCM.CLKCFG =
      (ADC12_CLKCFG_KEY_UNLOCK_W | ADC12_CLKCFG_CCONSTOP_DISABLE |
       ADC12_CLKCFG_CCONRUN_DISABLE | ADC12_CLKCFG_SAMPCLK_ULPCLK);
...
//   - Triggers the conversion process by setting the SC bit in the CTL1
//   register.
...
uint32_t ADC0_in(uint8_t channel) {
  ...
  ADC0->ULLMEM.MEMCTL[0] =
      ADC12_MEMCTL_WINCOMP_DISABLE | ADC12_MEMCTL_TRIG_AUTO_NEXT |
      ADC12_MEMCTL_BCSEN_DISABLE | ADC12_MEMCTL_AVGEN_DISABLE |
      ADC12_MEMCTL_STIME_SEL_SCOMP0 | ADC12_MEMCTL_VRSEL_VDDA_VSSA | channel;
...
#define BIAS_VOLTAGE (float)(3.30)
  ...
  celsius_temp = (COEFFICIENT_A4 * powf(voltage_temp, 4)) +
                 ...
                 (COEFFICIENT_A1 * voltage_temp) + (COEFFICIENT_A0);
